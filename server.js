...
// Short, final-feeling acknowledgment sent when the customer is just
// closing out the conversation — deliberately does NOT invite further
// chat (no "feel free to ask anything else"), since that's what was
// prompting customers to keep replying to an already-finished conversation.
const CLOSING_ACK_MESSAGE = "ߙ നന്ദി!";
...
// Detects a short message that is JUST closing out an already-finished
// conversation (thanks / ok / bye / a folded-hands emoji, etc.) rather than
// asking anything new. This is a plain regex check — no GPT call — used to
// avoid burning tokens on messages that don't need a generated reply.
// Deliberately conservative: bails out (returns false) on anything longer
// than a short phrase or containing a question mark, so a real question
// never gets silently swallowed. Worth noting this only short-circuits the
// GENERATED reply — it never skips the parts of the flow that read actual
// customer data (name/dob/gender extraction, payment confirmation, etc.).
function isClosingMessage(text) {
  const t = text.trim();
  if (!t || t.length > 40 || t.includes("?") || t.includes("?")) return false;
  const closingWord =
    /(ok+|okay|okk+|alright|fine|noted|got ?it|great|good|nice|super|perfect|thank(?:s| you)?(?: so much| very much)?|tnx|thanku|thnq|bye+|good ?night|good ?day|welcome|no(?: more)?(?: questions?)?|ശരി+|നന്ദി+|നന്ദിയുണ്ട്|സന്തോഷം|ആശംസകൾ|ശുഭരാത്രി|ശുഭദിനം|സൂപ്പർ|കൊള്ളാം|ߙ+|ߑ+|❤️+|ߘ+|ߘ️+|ߘ+|ߥ+)/;
  const closingPattern = new RegExp(
    `^(?:${closingWord.source}[\\s.,!।॥]*)+$`,
    "i"
  );
  return closingPattern.test(t);
}
  ...
  if (session.stage === "collecting") {
    ...
    if (!faqAnswer && Object.keys(patch).length === 0) {
      if (isClosingMessage(text)) {
        // Bare "ok"/"thanks"/etc — no need to burn a GPT call generating a
        // reassurance paragraph. Fall straight through to the normal
        // "please send your details" prompt below.
      } else {
...
End by asking them to share their പേര് (name), ജനനത്തീയതി (date of birth), and ലിംഗം (gender) together to continue.`,
  ...
  if (session.stage === "awaiting_payment") {
    ...
    if (isClosingMessage(text)) {
      // Bare "ok"/"thanks"/etc while waiting on payment — just repeat the
      // short payment reminder instead of burning a GPT call.
...
After your answer, end with a gentle reminder that once they complete the ₹99 payment using the QR code above, they should send the payment screenshot here to receive their reading.`,
  ...
  if (session.stage === "report_sent") {
    // Cheap, no-GPT check: if the customer is just closing out the
    // conversation (thanks/ok/bye/a folded-hands emoji, etc.) rather than
    // asking anything new, reply with a short fixed acknowledgment instead
    // of running the wantsAnotherPersonReading classifier AND a full
    // generated follow-up reply. Previously every single message here —
    // including a bare "ok" — triggered both GPT calls, which cost tokens
    // and often produced a reply that invited the customer to keep
    // chatting about a conversation they'd already finished.
    if (isClosingMessage(text)) {
      if (session.pendingSecondPerson) {
        await db.updateSession(phone, { pendingSecondPerson: false });
      }
      await sendText(phone, CLOSING_ACK_MESSAGE);
      return;
    }
...
- Do NOT habitually end your reply with an invitation to keep chatting (e.g. "feel free to ask anything else", "let me know if you have more questions"). Only invite further questions if the customer's message genuinely left something open. A short, complete reply that doesn't prompt another reply is preferred once the conversation has naturally reached a close.
${
  (session.orderCount || 1) > 1
...
// Admin: suspiciously-short "delivered" reports — GET /admin/suspicious-reports?key=resetmybot123
// Finds every session where report_status='sent' (looks like success) but
// report_text is far too short to be a genuine 2000+ word reading — the
// exact signature of a real incident where a Malayalam-language model
// refusal was delivered to a paying customer as if it were their report.
// report_status='failed' / /admin/failed-payments does NOT catch this,
// since these sessions look like successes in the DB.
app.get("/admin/suspicious-reports", async (req, res) => {
  const { key } = req.query;
  if (key !== RESET_COMMAND) {
    return res.status(403).send("Forbidden — missing or wrong key.");
  }
  ...
  try {
    const suspicious = await db.findSuspiciouslyShortReports();
    const rows = suspicious
      .map((s) => {
        const time = new Date(s.updatedAt).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
        const name = escapeHtml(s.name || "(no name)");
        const preview = escapeHtml((s.reportText || "").slice(0, 300));
        return `<a href="/admin/chats/view?phone=${encodeURIComponent(s.phone)}&key=${encodeURIComponent(key)}" style="text-decoration:none;color:inherit;">
          <div style="padding:12px 16px;border-bottom:1px solid #333;">
            <div style="display:flex;justify-content:space-between;">
              <strong>${escapeHtml(s.phone)}</strong>
            ...
            <div style="color:#aaa;font-size:14px;">${name} · ${(s.reportText || "").length} characters (a real report is normally 10,000+)</div>
            <div style="color:#ccc;font-size:13px;margin-top:2px;">${preview}${(s.reportText || "").length > 300 ? "..." : ""}</div>
          </div>
        </a>`;
      })
      .join("");
    ...
    res.status(200).send(`<!DOCTYPE html>
<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>Suspicious Reports</title></head>
<body style="background:#111;color:#eee;font-family:sans-serif;margin:0;">
  <div style="padding:16px;font-size:20px;font-weight:bold;border-bottom:1px solid #333;">Marked "sent" but suspiciously short (${suspicious.length})</div>
  <div style="padding:8px 16px;color:#888;font-size:13px;">These paid customers have report_status='sent' in the DB, but the content is too short to be a real reading — likely a model refusal that slipped through undetected. Tap any row to see the full chat and confirm.</div>
  ${rows || '<div style="padding:16px;color:#888;">None found — every "sent" report looks like a genuine full-length reading. ߎ</div>'}
</body></html>`);
  } catch (err) {
    log("Admin suspicious-reports list failed (caught):", err.message);
    res.status(500).send("Failed to load: " + err.message);
  }
});
